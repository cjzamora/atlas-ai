...
export async function execCommand({ args, flags }) {
  ...
  logger.finishRun(run.id, {
    ...
    metrics: {
      ...
      retryExhausted: Boolean(result.retry?.exhausted),
      finalErrorCode: result.error?.code || null,
      finalErrorProvider: result.error?.provider || null,
      finalErrorStatus: result.error?.status ?? null,
      finalErrorRetryable: result.error?.retryable ?? null,
      selectedTests: request.selectedTests.length,
